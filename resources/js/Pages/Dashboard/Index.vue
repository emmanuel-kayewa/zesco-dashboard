<template>
  <AppLayout :directorates="directorates">
    ...
    <!-- Compact Header Bar -->
    <div
      class="flex flex-wrap items-center justify-between gap-3 mb-6 no-print"
    >
      <div class="flex items-center gap-3">
        <!-- Tab Switcher -->
        <div
          class="flex items-center bg-gray-100 dark:bg-gray-700/50 rounded-lg p-0.5"
        >
          <button
            v-for="tab in tabs"
            :key="tab.key"
        ...
        <ChartCard title="Investment by Sector (USD)" :baseHeight="340">
          <template #default="{ zoomedHeight }">
            <BarChart
              ...
              :height="zoomedHeight"
              :multiSeries="[
                { name: 'Committed', field: 'committed', color: twoTone?.[0] },
                { name: 'Paid', field: 'paid', color: twoTone?.[1] },
              ]"
            />
          </template>
        </ChartCard>
        ...
        >
          ...
          >
            <div class="flex items-center gap-3 mb-3">
              ...
              <span
                v-if="!d.has_data"
                class="ml-auto text-[10px] text-gray-400 bg-gray-100 dark:bg-gray-700 px-1.5 py-0.5 rounded"
                >No data yet</span
              >
            </div>
            <p
              class="text-xs text-gray-500 dark:text-gray-400 mb-3 truncate"
              :title="d.name"
            >
              {{ d.name }}
            </p>
      ...
      <ChartCard title="Directorate Comparison" class="mb-6" :baseHeight="380">
        <template #default="{ zoomedHeight }">
          <BarChart
            ...
            :seriesName="
              comparisonMetrics.find((m) => m.key === selectedMetric)?.label ||
              ''
            "
            :horizontal="true"
            :height="zoomedHeight"
          ...
          <table class="w-full text-sm min-w-[640px]">
            <thead>
              <tr class="border-b border-gray-200 dark:border-gray-700">
                <th
                  class="text-left py-3 px-4 text-xs font-semibold text-gray-500 uppercase"
                >
                  Directorate
                </th>
                <th
                  class="text-right py-3 px-4 text-xs font-semibold text-gray-500 uppercase"
                >
                  Projects
                </th>
                <th
                  class="text-right py-3 px-4 text-xs font-semibold text-gray-500 uppercase hidden sm:table-cell"
                >
                  Committed (USD)
                </th>
                <th
                  class="text-right py-3 px-4 text-xs font-semibold text-gray-500 uppercase hidden md:table-cell"
                >
                  Paid (USD)
                </th>
                <th
                ...
                <th
                  class="text-center py-3 px-4 text-xs font-semibold text-gray-500 uppercase"
                >
                  Status
                </th>
              </tr>
            ...
            <tbody>
              <tr
                v-for="d in directorateSummaries"
                :key="d.id"
                class="border-b border-gray-100 dark:border-gray-700/50 hover:bg-gray-50 dark:hover:bg-gray-700/20 transition"
              >
                <td class="py-3 px-4">
                  <Link
                    :href="`/dashboard/directorate/${d.slug}`"
                    class="font-medium text-zesco-700 dark:text-zesco-400 hover:underline truncate block max-w-[200px]"
                    :title="`${d.code} — ${d.name}`"
                  >
                    {{ d.code }} &mdash; {{ d.name }}
                  </Link>
                ...
                <td class="text-right py-3 px-4">
                  <span
                    v-if="d.has_data"
                    :class="
                      d.high_risk_count > 2 ? 'text-red-600 font-semibold' : ''
                    "
                  >
                    {{ d.risk_count }} ({{ d.high_risk_count }} high)
                  </span>
                ...
                <td class="text-center py-3 px-4">
                  <span
                    ...
                    :class="{
                      ...
                      'bg-amber-500':
                        d.avg_progress >= 25 && d.avg_progress < 50,
                      'bg-red-500': d.avg_progress < 25,
                    }"
                  ></span>
                  <span
                    v-else
                    class="inline-block w-2.5 h-2.5 rounded-full bg-gray-300 dark:bg-gray-600"
                  ></span>
                </td>
              </tr>
            </tbody>
...
const { twoTone } = useChartPalettes();
...
const portfolioCostPie = computed(() => {
  ...
  return [
    { name: "Paid to Date", value: paid, color: twoTone.value?.[0] },
    { name: "Remaining", value: remaining, color: twoTone.value?.[1] },
  ];
});
