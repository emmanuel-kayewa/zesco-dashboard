    ...
    <div class="space-y-6">
      ...
      <div class="space-y-3">
        ...
        <div class="flex items-center justify-between">
          <Link :href="backUrl" class="btn-secondary text-sm px-4 py-2">
            &larr; Back
          </Link>
          <Link
            v-if="canEdit"
            :href="`/pp/weekly-reports/${report.id}/edit`"
        ...
        <div>
          ...
          <p
            class="text-base sm:text-lg font-semibold text-gray-600 dark:text-gray-400 mt-0.5"
          >
            Weekly Brief Report — Week {{ report.week_number }}
          </p>
          <div
            class="flex flex-wrap items-center gap-x-4 gap-y-1 mt-2 text-sm text-gray-500 dark:text-gray-400"
          >
      ...
      >
        ...
        <div
          v-if="activeTab === 'visual' && activeDrillFilter"
          class="flex items-center gap-2"
        >
          <span
            class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-300"
          >
            <span class="text-[10px] uppercase text-gray-500 dark:text-gray-400"
              >{{ activeDrillFilter.dimension }}:</span
            >
            {{ activeDrillFilter.value }}
            <button
              @click="clearDrill"
              class="ml-1 text-gray-400 hover:text-gray-700 dark:hover:text-white"
            >
              <svg
                class="w-3 h-3"
                fill="none"
          ...
          <Card>
            <template #title>
              <div class="flex items-center gap-3">
                <span
                  class="flex items-center justify-center w-8 h-8 rounded-lg bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-white text-sm font-bold flex-shrink-0"
                >
            ...
            <template v-if="section.section_type !== 'net_metering'">
              <div class="overflow-x-auto">
                <table class="w-full text-sm">
                  <thead>
                    <tr
                      class="border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/30"
                    >
                      <th
                        class="text-left py-2.5 px-3 text-xs font-semibold text-gray-500 uppercase"
                      >
                        Name of Project
                      </th>
                      <th
                      ...
                      <th
                        ...
                        class="text-right py-2.5 px-3 text-xs font-semibold text-gray-500 uppercase"
                      >
                        Size (MW)
                      </th>
                      <th
                  ...
                  <tbody>
                    <tr
                      v-for="entry in section.project_entries"
                      :key="entry.id"
                    ...
                    >
                      ...
                      >
                        ...
                        <p
                          v-if="entry.notes"
                          class="text-xs text-gray-400 mt-0.5 font-normal"
                        >
                          {{ entry.notes }}
                        </p>
                      </td>
                      ...
                      <td
                        v-if="section.section_type !== 'transmission_projects'"
                        class="py-2 px-3 text-right font-semibold text-gray-900 dark:text-white"
                      >
                        {{
                          entry.size_mw
                            ? Number(entry.size_mw).toLocaleString("en-US", {
                      ...
                      <td
                        v-if="section.section_type !== 'completed_solar'"
                        class="py-2 px-3 text-gray-600 dark:text-gray-300"
                      >
                        {{ entry.est_completion || "—" }}
                      </td>
                    </tr>
                  </tbody>
                  <tfoot
                  ...
                  >
                    <tr
                      class="border-t-2 border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-700/30"
                    >
                      <td
                        class="py-2.5 px-3 font-bold text-gray-900 dark:text-white"
                      >
                        TOTAL PROJECTS:
                        {{ section.project_entries?.length || 0 }}
                      </td>
                      <td colspan="2"></td>
                      <td
                        class="py-2.5 px-3 text-right font-bold text-gray-900 dark:text-white"
                      >
                        {{ sectionTotalMw(section) }}
                      </td>
                      <td
                        :colspan="
                          section.section_type === 'completed_solar' ? 1 : 2
                        "
                      ></td>
                    </tr>
                  </tfoot>
            ...
            <template v-else>
              <div class="overflow-x-auto">
                <table class="w-full text-sm">
                  ...
                  <tbody>
                    <tr
                      v-for="entry in section.net_metering_entries"
                      :key="entry.id"
                      class="border-b border-gray-100 dark:border-gray-700/50 hover:bg-gray-50 dark:hover:bg-gray-700/20"
                    >
                      <td
                        class="py-2 px-3 font-medium text-gray-900 dark:text-white"
                      >
                        {{ entry.key_initiative }}
                      </td>
                      <td
                        class="py-2 px-3 font-semibold text-gray-900 dark:text-white"
                      >
                        {{ entry.status_value }}
                      </td>
                    </tr>
      ...
      <template v-if="activeTab === 'visual'">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          >
            <p
              class="text-xs text-gray-500 dark:text-gray-400 uppercase font-semibold"
            >
              Completed Projects
            </p>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <ChartCard
            title="Under Construction — Capacity by Project"
            :baseHeight="320"
          >
            <template #default="{ zoomedHeight }">
              <BarChart
        ...
        <Card title="Net Metering Summary">
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div
              v-for="entry in netMeteringSection?.net_metering_entries || []"
              :key="entry.id"
        ...
        <Card
          v-if="activeDrillFilter"
          :title="`Filtered: ${activeDrillFilter.dimension} = ${activeDrillFilter.value}`"
        >
          <div class="overflow-x-auto">
            <table class="w-full text-sm">
              <thead>
...
const completedSection = computed(() =>
  sections.value.find((s) => s.section_type === "completed_solar"),
);
const constructionSection = computed(() =>
  sections.value.find((s) => s.section_type === "construction_projects"),
);
const transmissionSection = computed(() =>
  sections.value.find((s) => s.section_type === "transmission_projects"),
);
const netMeteringSection = computed(() =>
  sections.value.find((s) => s.section_type === "net_metering"),
);
...
const completedCount = computed(
  () => completedSection.value?.project_entries?.length || 0,
);
const completedMw = computed(() => sectionTotalMw(completedSection.value));
const constructionCount = computed(
  () => constructionSection.value?.project_entries?.length || 0,
);
const constructionMw = computed(() =>
  sectionTotalMw(constructionSection.value),
);
const transmissionCount = computed(
  () => transmissionSection.value?.project_entries?.length || 0,
);

const netMeteringProsumers = computed(() => {
  const entry = (netMeteringSection.value?.net_metering_entries || []).find(
    (e) => e.key_initiative?.toLowerCase().includes("prosumer"),
  );
...
function sectionTotalMw(section) {
  ...
  const total = section.project_entries.reduce(
    (sum, e) => sum + (Number(e.size_mw) || 0),
    0,
  );
...
const capacityByTypeData = computed(() => {
  const source = activeDrillFilter.value
    ? filteredEntries.value
    : allProjectEntries.value;
...
const constructionByProjectData = computed(() => {
  return (constructionSection.value?.project_entries || [])
    ...
    .slice(0, 15)
    .map((e) => ({
      label: (e.name || "Unknown").substring(0, 25),
      value: Number(e.size_mw) || 0,
    }));
});
...
const capacityByStatusData = computed(() => {
  ...
  for (const s of sections.value) {
    ...
    const mw = (s.project_entries || []).reduce(
      (sum, e) => sum + (Number(e.size_mw) || 0),
      0,
    );
  ...
  const items = [
    {
      ...
      href: !props.readOnly ? "/pp/weekly-reports" : "/weekly-reports",
    },
