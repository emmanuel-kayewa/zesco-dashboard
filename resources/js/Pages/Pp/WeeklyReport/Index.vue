        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-6">
                ...
                <div v-if="reports.data?.length" class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    ...
                    <Link
                        ...
                        class="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg bg-gray-900 dark:bg-white text-white dark:text-gray-900 text-sm font-medium hover:bg-gray-800 dark:hover:bg-gray-100 transition shadow-sm"
                    >
                ...
                <!-- Report Cards Grid -->
                <div v-if="reports.data?.length" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                    <WeeklyReportCard
                ...
                <!-- Pagination -->
                <div v-if="reports.links?.length > 3" class="flex items-center justify-center gap-1 pt-2">
                    <template v-for="link in reports.links" :key="link.label">
                        <Link v-if="link.url" :href="link.url" class="px-3 py-1.5 rounded-lg text-xs font-medium transition" :class="link.active ? 'bg-zesco-600 text-white shadow-sm' : 'hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-600 dark:text-gray-400'" v-html="link.label" />
