<template>
  <AppLayout
    ...
    :aiScope="{
      ...
      filters: explorerData.appliedFilters,
    }"
  >
        ...
        <template #left>
          <div class="min-w-0">
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
              {{ explorerData.totalCount }} project{{
                explorerData.totalCount !== 1 ? "s" : ""
              }}
              matching
    ...
    <div
      class="lg:hidden flex flex-nowrap items-center gap-2 mb-4 overflow-x-auto scrollbar-hide"
    >
      <span
      ...
      <template v-if="hasFilters">
        <span
          ...
          :key="dim"
          class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium flex-shrink-0 whitespace-nowrap"
          :style="filterChipStyle"
        >
          <span class="text-[10px] uppercase opacity-70"
            >{{ dimensionLabels[dim] || dim }}:</span
          >
          {{ val }}
          <button
            @click="removeFilter(dim)"
        ...
        <button
          ...
          class="text-xs text-red-500 hover:text-red-700 font-medium ml-1 flex-shrink-0"
        >
          Clear All
        </button>
      </template>
      <span v-else class="text-xs text-gray-400 italic flex-shrink-0"
    ...
    <div class="flex items-center justify-between gap-3 mb-6 no-print">
      ...
      <!-- View Mode Toggle (md screens only; single-col on mobile, sidebar on lg) -->
      <div
        class="hidden md:flex lg:hidden items-center gap-1 bg-gray-100 dark:bg-gray-700/50 rounded-lg p-1 flex-shrink-0"
      >
        <button
    ...
    <template v-if="viewMode === 'classic'">
      ...
      <template v-if="Object.keys(explorerData.breakdowns).length > 0">
        <div class="mb-2">
          ...
          <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
            Click any chart element to add a filter
          </p>
        </div>
            ...
            >
              <template #default="{ zoomedHeight }">
                ...
                >
                  <BarChart
                    :data="
                      bd.data.map((d) => ({ label: d.name, value: d.value }))
                    "
                    xField="label"
                    yField="value"
                    :seriesName="bd.label"
                    ...
                    @chart-click="
                      (p) =>
                        addDimensionFilter(dim, p.name || extractBarLabel(p))
                    "
                  />
                </ZambiaMapChart>
                ...
                <BarChart
                  ...
                  :horizontal="bd.data.length > 5"
                  @chart-click="
                    (p) => addDimensionFilter(dim, p.name || extractBarLabel(p))
                  "
                />
              </template>
            </ChartCard>
      ...
      <!-- <div
        v-if="explorerData.risksByCategory.length > 0"
    ...
    <template v-else-if="viewMode === 'compact'">
      <template
        v-if="
          Object.keys(explorerData.breakdowns).length > 0 ||
          explorerData.risksByCategory.length > 0
        "
      >
        <div class="mb-2">
          <h3 class="text-lg font-bold text-gray-900 dark:text-white">
        ...
        <div
          class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-6 lg:grid-cols-12 gap-6 mb-8"
        >
          ...
          <div class="col-span-1 sm:col-span-2 md:col-span-6 lg:col-span-6">
            <Card title="Projects" class="h-full">
              <template #actions>
                <button
                  @click="
                    expandedChart = { type: 'projects', title: 'Projects List' }
                  "
                ...
                <div class="flex items-center justify-between mb-3 px-1">
                  <p class="text-sm text-gray-600 dark:text-gray-400">
                    <span class="font-semibold text-gray-900 dark:text-white">{{
                      explorerData.projects.length
                    }}</span>
                    project{{ explorerData.projects.length !== 1 ? "s" : "" }}
                  </p>
                  <button
                    @click="
                      expandedChart = {
          ...
          <div
            v-if="
              explorerData.breakdowns.province ||
              explorerData.breakdowns.district
            "
          ...
          >
            ...
            >
              ...
              <template v-if="explorerData.breakdowns.province" #tab-province>
                <ZambiaMapChart
                  :data="
                    explorerData.breakdowns.province.data.map((d) => ({
                      name: d.name,
                      value: d.value,
                      investment: d.totalCost || 0,
                    }))
                ...
                >
                  <BarChart
                    ...
                    :colors="
                      pickCategorical(
                        explorerData.breakdowns.province.data.length,
                      )
                    "
                    height="280px"
                    :horizontal="
                      explorerData.breakdowns.province.data.length > 5
                    "
                    @chart-click="
                      (p) =>
                        addDimensionFilter(
                          'province',
                          p.name || extractBarLabel(p),
                        )
                    "
                  />
                </ZambiaMapChart>
              ...
              <template v-if="explorerData.breakdowns.district" #tab-district>
                <ZambiaMapChart
                  :data="
                    explorerData.breakdowns.district.data.map((d) => ({
                      name: d.name,
                      value: d.value,
                      investment: d.totalCost || 0,
                    }))
                ...
                >
                  <BarChart
                    ...
                    :colors="
                      pickCategorical(
                        explorerData.breakdowns.district.data.length,
                      )
                    "
                    height="280px"
                    :horizontal="
                      explorerData.breakdowns.district.data.length > 5
                    "
                    @chart-click="
                      (p) =>
                        addDimensionFilter(
          ...
          <div
            v-if="explorerData.breakdowns.developer"
            class="col-span-1 md:col-span-6 lg:col-span-4"
          >
            <ChartCard
              :class="[
                'h-full',
                explorerData.breakdowns.developer.isFiltered &&
                  'ring-2 ring-zesco-400/50',
              ]"
            >
              <template #title>
                <div class="flex items-center gap-2">
              ...
              <template #default="{ zoomedHeight }">
                <Pie3DChart
                  :data="
                    explorerData.breakdowns.developer.data.map((d) => ({
                      name: d.name,
                      value: d.value,
                    }))
                  "
          ...
          <div
            v-if="explorerData.breakdowns.sub_sector"
            class="col-span-1 md:col-span-6 lg:col-span-4"
          >
            <ChartCard
              :class="[
                'h-full',
                explorerData.breakdowns.sub_sector.isFiltered &&
                  'ring-2 ring-zesco-400/50',
              ]"
            >
              <template #title>
              ...
              <template #default="{ zoomedHeight }">
                <Pie3DChart
                  :data="
                    explorerData.breakdowns.sub_sector.data.map((d) => ({
                      name: d.name,
                      value: d.value,
                    }))
                  "
          ...
          <div
            v-if="explorerData.breakdowns.programme"
            class="col-span-1 md:col-span-6 lg:col-span-4"
          >
            <ChartCard
              :class="[
                'h-full',
                explorerData.breakdowns.programme.isFiltered &&
                  'ring-2 ring-zesco-400/50',
              ]"
            >
              <template #title>
              ...
              <template #default="{ zoomedHeight }">
                <BarChart
                  :data="
                    explorerData.breakdowns.programme.data.map((d) => ({
                      label: d.name,
                      value: d.value,
                    }))
                  "
                  ...
                  :colors="
                    pickCategorical(
                      explorerData.breakdowns.programme.data.length,
                    )
                  "
                  :height="zoomedHeight || '280px'"
                  :horizontal="true"
          ...
          >
            ...
            >
              <template #default="{ zoomedHeight }">
                <Pie3DChart
                  :data="
                    explorerData.breakdowns.sector.data.map((d) => ({
                      name: d.name,
                      value: d.value,
                    }))
                  "
          ...
          <div
            v-if="explorerData.breakdowns.contractor"
            class="col-span-1 md:col-span-6 lg:col-span-6"
          >
            <ChartCard
              :class="[
                'h-full',
                explorerData.breakdowns.contractor.isFiltered &&
                  'ring-2 ring-zesco-400/50',
              ]"
            >
              <template #title>
              ...
              <template #default="{ zoomedHeight }">
                <Pie3DChart
                  :data="
                    explorerData.breakdowns.contractor.data.map((d) => ({
                      name: d.name,
                      value: d.value,
                    }))
                  "
          ...
          <div
            v-if="explorerData.breakdowns.status"
            class="col-span-1 md:col-span-6 lg:col-span-6"
          >
            <ChartCard
              :class="[
                'h-full',
                explorerData.breakdowns.status.isFiltered &&
                  'ring-2 ring-zesco-400/50',
              ]"
            >
              <template #title>
              ...
              <template #default="{ zoomedHeight }">
                <BarChart
                  :data="
                    explorerData.breakdowns.status.data.map((d) => ({
                      label: d.name,
                      value: d.value,
                    }))
                  "
                  ...
                  :colors="
                    pickCategorical(explorerData.breakdowns.status.data.length)
                  "
                  :height="zoomedHeight || '280px'"
                  :horizontal="explorerData.breakdowns.status.data.length > 3"
                  @chart-click="
                    (p) =>
                      addDimensionFilter('status', p.name || extractBarLabel(p))
                  "
                />
              </template>
            </ChartCard>
      ...
      <div v-if="isIppContext" class="mb-8">
        <Link
          href="/pp/dashboard/grid-studies"
          class="block bg-gradient-to-r from-purple-50 to-blue-50 dark:from-purple-900/20 dark:to-blue-900/20 rounded-xl border border-purple-200 dark:border-purple-700/50 p-5 hover:shadow-lg transition-all duration-200 cursor-pointer group"
        >
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
                View the dedicated 5-stage pipeline tracker for IPP grid
                connection studies
              </p>
            </div>
            <svg
              class="w-5 h-5 text-gray-400 group-hover:text-purple-500 transition-colors flex-shrink-0"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
    ...
    <div v-if="viewMode === 'classic'" class="mb-6">
      <div class="mb-2">
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
          {{ explorerData.projects.length }} projects — click a project for full
          details
        </p>
      </div>
      ...
      <Card noPadding>
        <div class="p-4">
          ...
          <div v-if="filteredProjects.length === 0" class="text-center">
            <button
              @click="clearAllFilters"
              class="text-sm text-zesco-600 hover:underline"
            >
              Clear all filters
            </button>
          </div>
    ...
    <ChartModal
      :model-value="!!expandedChart"
      :title="expandedChart?.title || 'Chart'"
      @update:model-value="(val) => !val && (expandedChart = null)"
    >
      <!-- Sector Chart Expanded -->
      <Pie3DChart
        v-if="
          expandedChart?.type === 'sector' && explorerData.breakdowns.sector
        "
        :data="
          explorerData.breakdowns.sector.data.map((d) => ({
            name: d.name,
        ...
        @chart-click="
          (p) => {
            addDimensionFilter('sector', p.name);
            expandedChart = null;
          }
        "
      />
      ...
      <Pie3DChart
        v-else-if="
          expandedChart?.type === 'sub_sector' &&
          explorerData.breakdowns.sub_sector
        "
        :data="
          explorerData.breakdowns.sub_sector.data.map((d) => ({
            name: d.name,
        ...
        @chart-click="
          (p) => {
            addDimensionFilter('sub_sector', p.name);
            expandedChart = null;
          }
        "
      />
      ...
      <BarChart
        v-else-if="
          expandedChart?.type === 'status' && explorerData.breakdowns.status
        "
        :data="
          explorerData.breakdowns.status.data.map((d) => ({
            label: d.name,
      ...
      <BarChart
        v-else-if="
          expandedChart?.type === 'programme' &&
          explorerData.breakdowns.programme
        "
        :data="
          explorerData.breakdowns.programme.data.map((d) => ({
            label: d.name,
        ...
        @chart-click="
          (p) => {
            addDimensionFilter('programme', p.name || extractBarLabel(p));
            expandedChart = null;
          }
        "
      />
      ...
      <Pie3DChart
        v-else-if="
          expandedChart?.type === 'contractor' &&
          explorerData.breakdowns.contractor
        "
        :data="
          explorerData.breakdowns.contractor.data.map((d) => ({
            name: d.name,
        ...
        @chart-click="
          (p) => {
            addDimensionFilter('contractor', p.name);
            expandedChart = null;
          }
        "
      />
      ...
      <Pie3DChart
        v-else-if="
          expandedChart?.type === 'developer' &&
          explorerData.breakdowns.developer
        "
        :data="
          explorerData.breakdowns.developer.data.map((d) => ({
            name: d.name,
        ...
        @chart-click="
          (p) => {
            addDimensionFilter('developer', p.name);
            expandedChart = null;
          }
        "
      />
      ...
      <div v-else-if="expandedChart?.type === 'geographic'" class="space-y-6">
        <TabCard title="Geographic Distribution" :tabs="geographicTabs">
          <template #tab-province v-if="explorerData.breakdowns.province">
            <ZambiaMapChart
              ...
              @region-click="
                (name) => {
                  addDimensionFilter('province', name);
                  expandedChart = null;
                }
              "
            >
              <BarChart
                ...
                :seriesName="explorerData.breakdowns.province.label"
                :colors="
                  pickCategorical(explorerData.breakdowns.province.data.length)
                "
                height="600px"
          ...
          <template #tab-district v-if="explorerData.breakdowns.district">
            <ZambiaMapChart
              ...
              @region-click="
                (name) => {
                  addDimensionFilter('district', name);
                  expandedChart = null;
                }
              "
            >
              <BarChart
                ...
                :seriesName="explorerData.breakdowns.district.label"
                :colors="
                  pickCategorical(explorerData.breakdowns.district.data.length)
                "
                height="600px"
      ...
      <div v-else-if="expandedChart?.type === 'risk'" class="space-y-6">
        <TabCard title="Risk Analysis" :tabs="riskTabs">
          <template #tab-category>
            <Pie3DChart :data="riskCategoryData" height="600px" />
          </template>
          <template #tab-status>
            <BarChart
              :data="riskStatusData"
...
const Pie3DChart = defineAsyncComponent(
  () => import("@/Components/Charts/Pie3DChart.vue"),
);
const ZambiaMapChart = defineAsyncComponent(
  () => import("@/Components/Charts/ZambiaMapChart.vue"),
);
...
watch(
  () => props.explorerData.appliedFilters?.province,
  (provinceFilter) => {
    if (
      provinceFilter &&
      props.explorerData.breakdowns?.district &&
      !props.explorerData.breakdowns?.district?.isFiltered
    ) {
...
const activeGeographicFilter = computed(() => {
  if (props.explorerData.breakdowns?.province?.isFiltered) {
    return {
      label: "Province",
      value: props.explorerData.breakdowns.province.activeFilter,
    };
  }
  if (props.explorerData.breakdowns?.district?.isFiltered) {
    return {
      label: "District",
      value: props.explorerData.breakdowns.district.activeFilter,
    };
  }
  return null;
});
...
const headerMetrics = computed(() => {
  const spendColor =
    (props.explorerData.kpis?.spendPct ?? 0) >= 50 ? RAG.green : RAG.amber;
  const progressColor =
    (props.explorerData.kpis?.avgProgress ?? 0) >= 50 ? RAG.green : RAG.amber;
  return [
    { label: "Projects", value: props.explorerData.kpis?.totalProjects ?? 0 },
    {
      label: "Committed",
      value: `$${fmtM(props.explorerData.kpis?.totalCommitted)}`,
      color: INVESTMENT.committed,
    },
    {
      label: "Spend",
      value: `${props.explorerData.kpis?.spendPct ?? 0}%`,
      color: spendColor,
    },
    {
      label: "Avg Progress",
      value: `${props.explorerData.kpis?.avgProgress ?? 0}%`,
      color: progressColor,
    },
  ];
});
...
const breadcrumbItems = computed(() => {
  ...
  for (const [dim, val] of Object.entries(
    props.explorerData.appliedFilters || {},
  )) {
...
const hasFilters = computed(
  () => Object.keys(props.explorerData.appliedFilters || {}).length > 0,
);
...
const filterSummaryTitle = computed(() => {
  const parts = Object.entries(props.explorerData.appliedFilters || {}).map(
    ([dim, val]) => val,
  );
  return parts.join(" › ");
});
...
const addFilterOptions = computed(() => {
  ...
  return (props.explorerData.filterOptions?.[addFilterDim.value] || []).map(
    (opt) => ({
      value: opt,
      label: opt,
    }),
...
const filteredProjects = computed(() => {
  ...
  list.sort((a, b) => {
    ...
    if (sortField.value === "progress_pct")
      return (b.progress_pct || 0) - (a.progress_pct || 0);
    if (sortField.value === "capacity_mw")
